...
func (LinkStorage *LinkStorage) Insert(link models.Link) (bson.ObjectId, error) {
	...
	utils.DebugLog.Printf("Inserted new Link with ID %s into database.", link.ID.String())
	return link.ID, nil
}
...
func (LinkStorage *LinkStorage) Update(link models.Link) error {
	...
	utils.DebugLog.Printf("Updated Link with ID %s in database.", link.ID.String())
	return nil
}
...
func (LinkStorage *LinkStorage) GetOne(id bson.ObjectId) (models.Link, error) {
	...
	utils.DebugLog.Printf("Retrieved Link with ID %s from database.", link.ID.String())
	return *link, nil
}
